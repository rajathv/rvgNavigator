...
connectBTButton.setVisibility(View.GONE);
        closeBTButton.setVisibility(View.GONE);
...
try {
    Request request = new Request.Builder()
            .url("http://192.168.43.26/") // The URL to send the data to
            .post(formBody)
            .build();
}
catch (Throwable t){
...
// Move map camera to the selected location
                ...
                DESTINATION = Point.fromLngLat(((Point) selectedCarmenFeature.geometry()).latitude(),
                        ((Point) selectedCarmenFeature.geometry()).longitude());
